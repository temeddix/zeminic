<template>
  <div class="topBar" :style="outerStyle">
    ...
    >
      <div class="fillParent" style="background-image: url('/logo.png')"></div>
    </elastic-button>
...
-->
        ...
        <div v-else>
          ...
          <layout-block>
            <div>
              <elastic-button class="blackButton" @click.native="signOut">
                로그아웃
              </elastic-button>
              <elastic-button
    ...
    <elastic-button ref="debuggingPanel" class="debuggingPanel" poppable>
      <template v-slot:popup-content>
        디버깅 패널이다!
        <br />원하는 거 다 적어봐라 <br />Shift+D를 누르면 보인다
        <br />
...
export default {
  ...
  computed: {
    outerStyle() {
      return {
        top:
          this.tweenedTopBarPosition < 0
            ? this.tweenedTopBarPosition + "px"
            : "0px",
  ...
  mounted: function () {
    ...
    document.addEventListener("scroll", function () {
      self.targetTopBarPosition = this.isVisible
        ? 0
        : -Math.min(window.scrollY, 200);
    ...
    document.addEventListener("keydown", function (evt) {
      ...
      if (evt.shiftKey && evt.keyCode == 68) {
        //D+P https://www.quirksmode.org/js/keys.html
