<template>
  ...
  >
    ...
    <div class="overfillParent baseHighlight"></div>

    <div class="overfillParent">
      <div
        class="mouseHighlight overfillParent"
    ...
    >
      <div class="fillParent">
        <elastic-button class="exitButton" @click.native="hidePopup">
        </elastic-button>
        <vue-scroll>
          <div class="fillParent popupContent">
...
const hexToRgb = window.hexToRgb;
const detectMobile = window.detectMobile;
const becomeNew = window.becomeNew;
...
export default {
  ...
  computed: {
    buttonStyle() {
      return {
        ...
        boxShadow: this.switchOn
          ? "0px 0px 16px rgba(" +
            hexToRgb(this.switchColor).red +
            "," +
            hexToRgb(this.switchColor).green +
            "," +
            hexToRgb(this.switchColor).blue +
            ", 0.6)"
          : "",
  ...
  methods: {
    showPopup() {
      ...
      if (this.poppable) {
        ...
        becomeNew(
          self.$refs.buttonElement,
          self.$refs.popupElement,
          self.popDuration,
          true
        );
  ...
  created: function () {
    ...
    this.targetPopupHeight = Math.min(
      this.popupHeight,
      window.innerHeight - 40
    );
    this.targetPopupWidth = Math.min(this.popupWidth, window.innerWidth - 40);
    this.targetPopupPositionTop =
      (window.innerHeight - this.targetPopupHeight) / 2;
    this.targetPopupPositionLeft =
      (window.innerWidth - this.targetPopupWidth) / 2;

    window.addEventListener("resize", function () {
      self.targetPopupHeight = Math.min(
        self.popupHeight,
        window.innerHeight - 40
      );
      self.targetPopupWidth = Math.min(self.popupWidth, window.innerWidth - 40);
      self.targetPopupPositionTop =
        (window.innerHeight - self.targetPopupHeight) / 2;
      self.targetPopupPositionLeft =
        (window.innerWidth - self.targetPopupWidth) / 2;
    });
  },
  mounted: function () {
    let self = this;
    ...
    this.$refs.switchIndicator.style.borderRadius =
      parseInt(getComputedStyle(self.$refs.buttonElement).borderTopLeftRadius) -
      0 +
      "px";
