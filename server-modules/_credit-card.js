...
router.post('/ajax-tunnel/create-payment-method', function (req, res) {
    ...
    let pwd2digit = req.body.pwd2digit;
    ...
    if(expireDate.includes("-")){
        let year = Number(expireDate.split("-")[0]);
    ...
    if (idCard.code != 200) {
        console.log("/create_payment-method cannot identify card type :", idCard.code);
    }
