...
export default {
    ...
    computed: {
        balloonStyle() { return {
            ...
            top: ( this.elementPosition.top + (this.showUpward? 0: this.elementPosition.height) ) + 'px',
            left: ( this.elementPosition.left + (this.elementPosition.width/2) ) + 'px',
            transform: 'scale('+ (Math.sqrt(this.tweenedActivation)) + ') translateX(-50%) '
            + ' translateX(' + this.xCorrection + 'px)'
        ...
        pokeStyle(){ return {
            ...
            transform: (this.showUpward?'':' rotate(180deg)') + 'translateX(-50%)',
            transformOrigin: '0% 0%'
        } }
    },
    methods:{
    ...
    created: function(){
        ...
        if( self.elementPosition.top > window.innerHeight - self.elementPosition.top - self.elementPosition.height ){
            self.showUpward = true;
        }

        if( this.elementPosition.left + (this.elementPosition.width/2) - (this.balloonWidth/2) < 12){
            self.xCorrection = 12- ( this.elementPosition.left + (this.elementPosition.width/2) - (this.balloonWidth/2) );
        }
        else if( window.innerWidth - this.elementPosition.left - (this.elementPosition.width/2) - (this.balloonWidth/2) < 12 ){
            self.xCorrection = ( window.innerWidth - this.elementPosition.left - (this.elementPosition.width/2) - (this.balloonWidth/2) ) - 12;
        }
...
<style module>
    ...
    .roundHighlight{
        background: radial-gradient( circle at 50% 20%, rgba(255,255,255,0.2), rgba(255,255,255,0) );
    }
</style>